"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
Object.defineProperty(exports, "__esModule", { value: true });
var core_1 = require("@angular/core");
var obat_batch_service_1 = require("./obat-batch.service");
var DaftarObatComponent = (function () {
    function DaftarObatComponent(ObatBatchService) {
        this.ObatBatchService = ObatBatchService;
    }
    DaftarObatComponent.prototype.ngOnInit = function () {
        var _this = this;
        this.ObatBatchService.getAllObatBatch()
            .then(function (allObatBatch) { return _this.allObatBatch = allObatBatch; });
    };
    return DaftarObatComponent;
}());
DaftarObatComponent = __decorate([
    core_1.Component({
        selector: 'daftar-obat-page',
        templateUrl: './daftar-obat.component.html',
        providers: [obat_batch_service_1.ObatBatchService]
    }),
    __metadata("design:paramtypes", [obat_batch_service_1.ObatBatchService])
], DaftarObatComponent);
exports.DaftarObatComponent = DaftarObatComponent;
//# sourceMappingURL=daftar-obat.component.js.map